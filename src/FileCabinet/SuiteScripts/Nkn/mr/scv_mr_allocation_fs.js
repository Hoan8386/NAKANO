...
define(['N/format', 'N/record', 'N/runtime', 'N/search', '../lib/scv_lib_function.js'],
    ...
    (format, record, runtime, search, libFunc) => {
        ...
        const getInputData = () => {
            ...
            let template = currentScript.getParameter({name: 'custscript_scv_mr_altemplate'});
            let year_month = currentScript.getParameter({name: 'custscript_scv_mr_year_month'});
            //let datebasis = currentScript.getParameter({name : 'custscript_scv_mr_datebasis'});
            let timezone = currentScript.getParameter({name: 'custscript_scv_mr_timezone'});
            let trandate = currentScript.getParameter({name: 'custscript_scv_mr_trandate'});
            let ordertype = currentScript.getParameter({name: 'custscript_scv_mr_ordertype'});
            let memo = currentScript.getParameter({name: 'custscript_scv_mr_memo'});
            let subsidiary = currentScript.getParameter({name: 'custscript_scv_mr_fs_subsidiary'});
            let delimiter = /\u0005/;
        ...
        const makeLineTemplate = (template, year_month, timezone, trandate, ordertype, memo, subsidiary) => {
            ...
            for (let i = 0; i < lcAllcationDetail; i++) {
                ...
                arrAllcationDetail.push({
                    valueDest: valuedest, valuedestid: valuedestid, fieldid: fieldid, filterh: filterh, dest: dest
        ...
        const createAllocationEntry = (lineTemplate) => {
            ...
            pushFilter(fTieuThuc, lineTemplate.hdaterule, lineTemplate.timezone, lineTemplate.trandate, lineTemplate.subsidiary, lineTemplate.hfdateid, lineTemplate.hfsubsidiaryid, lineTemplate.hisnonsub);
            ...
            if (total !== 0) {
                let line = 0;
                let recAllocationEntry = record.create({type: 'customtransaction_scv_allocation'});
