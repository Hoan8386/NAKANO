...
const _scvForm = {
    ...
    initSublistPagination: function () {
        if(typeof _scvFormSublist == "undefined") return;

        this.sublists.forEach(objSublist =>{
            let pagination = objSublist?.pagination ?? {enable: true, pageSize: 1000};
            if(!pagination?.enable) return;

            if(this.currentRecord.getLineCount(objSublist.id) > 0) return;

            _scvFormSublist.initSublistPage(objSublist.id, {
                ...pagination, 
    ...
    ajax: {
        ...
        postWaitQueue: async function(_urlReq, _arrReqAction, _idxExcute = 0, 
            ...
            _scvForm.updateProgessStatus("Processing step: " + nextExcute + "/" + _arrReqAction.length);
