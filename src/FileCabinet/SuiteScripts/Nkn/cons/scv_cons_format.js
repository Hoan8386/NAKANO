...
(format, formati18n, query,
    ...
    const formatNumber = (_num, _fixed = 2, _options = {
		groupSeparator: '.',
		decimalSeparator: ',',
	}) => {
		if(typeof _num === 'number' && _num % 1 !== 0 && _fixed) {
	...
	const formatNumberWithObject = (_inputObj, _fixed = 2, _options = {
		groupSeparator: '.',
		decimalSeparator: ',',
	}) => {
		Object.keys(_inputObj).forEach(ele => {
			if(!!_inputObj[ele] && !isNaN(_inputObj[ele])){
				_inputObj[ele] = formatNumber(_inputObj[ele], _fixed, _options);
			}
		});
    ...
    const calcNumberMonths = (_d1, _d2, _isAbs = true) =>{
        if(!_d1 || !_d2) return 0;

        let d1 = typeof(_d1) == "string" ? parseDate(_d1) : _d1;
        let d2 = typeof(_d2) == "string" ? parseDate(_d2) : _d2;

        let totalMonths1 = d1.getFullYear() * 12 + d1.getMonth();
        let totalMonths2 = d2.getFullYear() * 12 + d2.getMonth();

        let diffMonth = Math.round(totalMonths2 - totalMonths1);

        return _isAbs ? Math.abs(diffMonth) : diffMonth;
    ...
    const getDateTimeWithCurrentGMT = (_dateTime) =>{
        let diffHours = query.runSuiteQL({query: `SELECT (SYSDATE - CURRENT_DATE) * 24 as diff_hours 
    ...
    const getMonthRanges = (_fromDate, _toDate) =>{
        let fromDate = !!_fromDate ? parseDate(_fromDate) : new Date(2020, 0, 1);
        let toDate = parseDate(_toDate);
        ...
        while (current <= toDate) {
            ...
            // Giới hạn nếu nằm ngoài khoảng fromDate/toDate
            if (startOfMonth < fromDate) startOfMonth = new Date(fromDate);
            if (endOfMonth > toDate) endOfMonth = new Date(toDate);
    ...
    const formatFileSize = (_size) => {
        let size = parseInt(_size, 10);
        if(isNaN(size)) return "";

        let arrUnits = ["B", "KB", "MB", "GB", "TB"];
        let idxUnit = 0;
        while(size >= 1024 && idxUnit < arrUnits.length - 1){
            size = size / 1024;
            idxUnit++;
        }

        return (Math.round(size * 100) / 100) + " " + arrUnits[idxUnit];
    }
