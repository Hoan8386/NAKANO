/**
 * Nội dung: 
 * + HuyPQ 20260820: Phần này cân nhắc sẽ bỏ DX + JQX => nên không sử dụng ở site này
 * Version: 1.260813.40
 * =======================================================================================
...
(serverWidget, runtime, query, search, message, file, url,
    lbf,
    alasql,
    constRecord,
    constFile,
    ...
    const addCommonClientScript = () => {
        ...
        Object.values(RECORDS.sublist).forEach(objSublist => {
            let objSublistTemp = {
                id: objSublist.id,
                pagination: objSublist.pagination,
                treeList: objSublist.treeList,
                columns: []
            };
            ...
            objSublist.columns.forEach(objColumn => {
                objSublistTemp.columns.push({
                    ...
                    displayType: objColumn.displayType,
                });
            });
        ...
        if(util.isObject(_webWorkerScripts)){
            ...
            _webWorkerScripts.forEach(objWebWorkerScript => {
                Object.keys(objWebWorkerScript).forEach(scriptName => {
                    RECORDS.webWorker[scriptName] = objWebWorkerScript[scriptName];
                });
            });
        }
    }
    ...
    const enableChart = () =>{
        addLibaryInclude([
            '../css/scv_chartjs.css',
            '../scripts/scv_chartjs.js',
            '../olib/chart.umd.min.js',
        ]);
    }

    const enableSublistPagination = () =>{
        addLibaryInclude([
            '../scripts/scv_form_sublist.js',
        ]);
    }
    ...
    const createForm = (_options, _clientScript, _optionUpdate = {}) => {
        ...
        optionUpdate.pinHeaderSublist = optionUpdate.pinHeaderSublist??false;
        optionUpdate.noWrapHeaderSublist = optionUpdate.noWrapHeaderSublist??true;
        optionUpdate.noWrapLineSublist = optionUpdate.noWrapLineSublist??false;
        optionUpdate.chart = optionUpdate.chart ?? false;
        optionUpdate.pagination = optionUpdate.pagination ?? false;
        ...
        if(optionUpdate.chart){
            enableChart();
        }
        
        if(optionUpdate.pagination){
            enableSublistPagination();
        }
        return RECORDS.form;
    }
    ...
    const addField = (_options, _isMandatory = false, _optionUpdate) =>{
        ...
        if(["select", "multiselect"].includes(_options.type) && optionUpdate?.lookup?.data?.length > 0){
            constRecord.initLoadFieldServer(field, optionUpdate.lookup, _options.type == "select");
        }
    ...
    const addSublist = (_options) =>{
        let columns = _options?.columns?.length > 0 ? _options.columns : [];

        let pagination = _options?.pagination ? _options?.pagination : {};
        pagination.enable = pagination?.enable ?? true;
        pagination.pageSize = pagination?.pageSize ?? 1000;

        delete _options.columns;
        delete _options.pagination;
        ...
        if(_options?.treeList?.enable){
            ...
            treeList.rootValue = treeList.rootValue ?? -1;
            treeList.keyExpr = treeList.keyExpr ?? "idxRow";
            treeList.parentIdExpr = treeList.parentIdExpr ?? "idxRowParent";
            treeList.autoExpandAll = treeList.autoExpandAll ?? true;

            RECORDS.sublist[_options.id].treeList = treeList
        }

        if(columns.length > 0){
            addFieldOfSublist(_options.id, columns);
        }
    ...
    const addFieldOfSublist = (_sublistId, _columns) => {
        ...
        for(let i = 0; i< _columns.length; i++){
            ...
            let displayType = objCol.displayType||"inline";
    ...
    const addPageLink = (_lstLink, isShowCurSuitelet = false) => {
		...
		if(isShowCurSuitelet){
			_lstLink.push({id: curScript.deploymentId, name: "Script Deployment Current"})
		}
