...
function(record,
    ...
    const initQuickFindSublistFieldSelect = (_curRec, _sublistId, _fieldId, _line, _lookup) =>{
        ...
        if(currentDropdown._scvCustomDropdown){
            let btnActionFs_org = _sublistId + "_" + _fieldId;

            if(!_lookup.isMachine){
                btnActionFs_org += (Number(_line) + 1);
            }

            btnActionFs_org += "_fs";

            let btnActionFs = "parent_actionbuttons_" + btnActionFs_org;
            let btnActionFsTooltipMenu = btnActionFs_org + "_tooltipMenu";

            jQuery("#" + btnActionFs + " a").off("click");
            jQuery("#" + btnActionFs + " a").on("click", function(event){
                jQuery("#" + btnActionFsTooltipMenu).css('display', 'none')
